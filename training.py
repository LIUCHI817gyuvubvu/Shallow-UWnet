...
class Trainer:
    ...
    def train(self, train_dataloader, config, test_dataloader = None):
        ...
        for epoch in trange(0, config.num_epochs, desc=f"[Full Loop]", leave=False):
            ...
            wandb.log({
                f"[Train] Total Loss": total_loss_lst[epoch],
            ...
            if epoch % config.print_freq == 0:
                print(f'epoch:[{epoch}]/[{config.num_epochs}], image loss:{total_loss_lst[epoch]}, MSE / L1 loss:{primary_loss_lst[epoch]}, VGG loss:{vgg_loss_lst[epoch]}')

            if not os.path.exists(config.snapshots_folder):
                os.mkdir(config.snapshots_folder)

            if epoch % config.snapshot_freq == 0:
                torch.save(self.model.state_dict(), config.snapshots_folder + f'model.ckpt')
